...
class ValiditySidechainDecomposedProfileReclaimReorg(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        supported = get_supported_profile(n0, "groth16_bls12_381_poseidon_v2")
        sidechain_id = 59
        ...
        self.log.info("Building shared v2 sidechain history with one reclaimable deposit.")
        ...
        self.log.info("On node0 only, reclaim the matured v2 deposit.")
        ...
        assert_equal(sidechain_n0["current_withdrawal_root"], initial_withdrawal_root)
        ...
        self.log.info("After reorg, the deposit should be reclaimable again and the v2 withdrawal root must remain intact.")
        ...
        assert_equal(sidechain_n0["current_withdrawal_root"], initial_withdrawal_root)
        assert_equal(sidechain_n1["current_withdrawal_root"], initial_withdrawal_root)
        assert_equal(sidechain_n0["escrow_balance"], amount_to_sats(deposit_amount))
        assert_equal(sidechain_n1["escrow_balance"], amount_to_sats(deposit_amount))
        ...
        assert_equal(sidechain_after_restart["current_withdrawal_root"], initial_withdrawal_root)
        ...
        self.log.info("After restart, the orphaned reclaim should either return to mempool or be resubmittable.")
        mempool = n0.getrawmempool()
        if reclaim_txid in mempool:
            self.log.info("The original v2 reclaim transaction was restored to mempool after the reorg.")
        else:
            reclaim_result = n0.sendstaledepositreclaim(
                sidechain_id,
                deposit_metadata,
        ...
        assert_equal(sidechain_final["current_withdrawal_root"], initial_withdrawal_root)
...
if __name__ == "__main__":
    ValiditySidechainDecomposedProfileReclaimReorg().main()
