...
class ValiditySidechainRealProfileReorg(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Building shared real-profile history on both nodes.")
        n0.sendvaliditysidechainregister(sidechain_id, config)
        n0.generatetoaddress(1, n0.getnewaddress())
        self.sync_blocks()

        for deposit in real_valid_vector.get("setup_deposits", []):
        ...
        if real_valid_vector.get("setup_deposits", []):
            n0.generatetoaddress(1, n0.getnewaddress())
            self.sync_blocks()
        ...
        assert_equal(
            shared_sidechain["queue_state"]["root"],
            pad_field_hex(real_valid_vector["public_inputs"]["l1_message_root_before"]),
        )

        self.disconnect_nodes(0, 1)

        self.log.info("On node0 only, accept the committed real-profile batch and execute its verified withdrawal.")
        real_public_inputs = {
        ...
        assert_equal(
            ...
            amount_to_sats(Decimal(real_valid_vector["setup_deposits"][0]["amount"]) - Decimal(withdrawal_rpc_entries[0]["amount"])),
        )

        self.log.info("Mine a longer competing fork on node1 that omits the real-profile batch and withdrawal.")
        ...
        self.log.info("After reorg, the committed batch, queue consumption, and executed-withdrawal nullifier must roll back.")
        ...
        assert_equal(sidechain_n0_after["queue_state"]["pending_message_count"], len(real_valid_vector["setup_deposits"]))
        assert_equal(sidechain_n1_after["queue_state"]["pending_message_count"], len(real_valid_vector["setup_deposits"]))
        assert_equal(
            sidechain_n0_after["escrow_balance"],
            amount_to_sats(Decimal(real_valid_vector["setup_deposits"][0]["amount"])),
        )
        assert_equal(
        ...
        assert_equal(sidechain_after_restart["queue_state"]["pending_message_count"], len(real_valid_vector["setup_deposits"]))

        self.log.info("Re-submitting the same committed batch and withdrawal after reorg should succeed.")
        ...
        assert_equal(
            final_sidechain["verified_withdrawal_execution_mode"],
            "withdrawal_root_single_leaf_experimental",
        )
        assert_equal(
...
if __name__ == "__main__":
    ValiditySidechainRealProfileReorg().main()
