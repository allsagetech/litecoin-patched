...
class ValiditySidechainForceExitConsumedReorg(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.log.info("Building shared sidechain history with a single force-exit request that matures on both nodes.")
        ...
        mature_height = request_height + config["force_inclusion_delay"]
        ...
        assert_equal(shared_sidechain["latest_batch_number"], 0)
        assert_equal(shared_sidechain["queue_state"]["head_index"], 0)
        assert_equal(shared_sidechain["queue_state"]["pending_message_count"], 1)
        assert_equal(shared_sidechain["queue_state"]["pending_force_exit_count"], 1)
        assert_equal(shared_sidechain["queue_state"]["matured_force_exit_count"], 1)
        ...
        self.log.info("On node0 only, mine a batch that consumes the matured force-exit request.")
        ...
        batch_result = n0.sendvaliditybatch(sidechain_id, public_inputs)
        ...
        assert_equal(sidechain_n0["accepted_batches"][0]["consumed_queue_messages"], 1)
        assert_equal(sidechain_n0["accepted_batches"][0]["queue_prefix_commitment"], public_inputs["queue_prefix_commitment"])
        ...
        self.log.info("On node1 only, mine a longer competing fork that omits the consuming batch.")
        ...
        self.log.info("After reorg, the consumed batch must roll back and the matured force-exit request must reappear.")
        ...
        assert_equal(sidechain_n0_after["queue_state"]["pending_force_exit_count"], 1)
        assert_equal(sidechain_n1_after["queue_state"]["pending_force_exit_count"], 1)
        assert_equal(sidechain_n0_after["queue_state"]["matured_force_exit_count"], 1)
        assert_equal(sidechain_n1_after["queue_state"]["matured_force_exit_count"], 1)
        ...
        assert_equal(sidechain_after_restart["queue_state"]["pending_force_exit_count"], 1)
        assert_equal(sidechain_after_restart["queue_state"]["matured_force_exit_count"], 1)

        self.log.info("After restart, the orphaned consuming batch should either be restored to mempool or be resubmittable.")
        mempool = n0.getrawmempool()
        if batch_txid in mempool:
            self.log.info("The original consuming batch was restored to mempool after the reorg.")
        else:
            batch_result = n0.sendvaliditybatch(sidechain_id, public_inputs)
            batch_txid = batch_result["txid"]
        ...
        assert_equal(final_sidechain["accepted_batches"][0]["consumed_queue_messages"], 1)
        assert_equal(final_sidechain["accepted_batches"][0]["queue_prefix_commitment"], public_inputs["queue_prefix_commitment"])
        assert_equal(final_sidechain["queue_state"]["head_index"], 1)
        assert_equal(final_sidechain["queue_state"]["pending_message_count"], 0)
        assert_equal(final_sidechain["queue_state"]["pending_force_exit_count"], 0)
        assert_equal(final_sidechain["queue_state"]["matured_force_exit_count"], 0)


if __name__ == "__main__":
    ValiditySidechainForceExitConsumedReorg().main()
