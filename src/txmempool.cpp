...
static bool ComputeSidechainTxInputCredit(const CTransaction& tx, CAmount& out_credit)
{
    ...
    if (escape_exit_marker_index != -1) {
        ...
        return MatchValidityEscapeExitPayouts(tx, escape_exit_marker_index, exit_proofs, out_credit);
    }
...
static bool TryGetValiditySidechainDepositKeyFromTx(const CTransaction& tx, std::pair<uint8_t, uint256>& out_key)
{
    int deposit_count = 0;
...
static bool TryGetValiditySidechainReclaimKeyFromTx(const CTransaction& tx, std::pair<uint8_t, uint256>& out_key)
{
    int reclaim_count = 0;
...
static bool TryGetValiditySidechainEscapeExitKeysFromTx(
    const CTransaction& tx,
    std::vector<std::pair<uint8_t, uint256>>& out_keys)
{
    int execute_count = 0;
...
void CTxMemPool::addUnchecked(const CTxMemPoolEntry &entry, setEntries &setAncestors, bool validFeeEstimate)
{
    ...
    if (TryGetValiditySidechainDepositKeyFromTx(tx, validity_deposit_key)) {
        const bool inserted = mapValiditySidechainDepositById.emplace(validity_deposit_key, tx.GetHash()).second;
        assert(inserted);
    }

    std::pair<uint8_t, uint256> validity_reclaim_key;
    if (TryGetValiditySidechainReclaimKeyFromTx(tx, validity_reclaim_key)) {
        const bool inserted = mapValiditySidechainReclaimByDepositId.emplace(validity_reclaim_key, tx.GetHash()).second;
        assert(inserted);
    }
    ...
    if (TryGetValiditySidechainEscapeExitKeysFromTx(tx, validity_escape_exit_keys)) {
        for (const auto& key : validity_escape_exit_keys) {
            const bool inserted = mapValiditySidechainEscapeExitById.emplace(key, tx.GetHash()).second;
            assert(inserted);
        }
    }
...
void CTxMemPool::removeUnchecked(txiter it, MemPoolRemovalReason reason)
{
    ...
    std::pair<uint8_t, uint256> validity_deposit_key;
    if (TryGetValiditySidechainDepositKeyFromTx(*ptx, validity_deposit_key)) {
        mapValiditySidechainDepositById.erase(validity_deposit_key);
    }
    ...
    std::vector<std::pair<uint8_t, uint256>> validity_escape_exit_keys;
    if (TryGetValiditySidechainEscapeExitKeysFromTx(*ptx, validity_escape_exit_keys)) {
        for (const auto& key : validity_escape_exit_keys) {
            mapValiditySidechainEscapeExitById.erase(key);
        }
    }
...
void CTxMemPool::_clear()
{
    ...
    mapValiditySidechainEscapeExitById.clear();
    ...
    const int64_t spendheight)
{
    ...
    CAmount txfee = 0;
    CAmount input_credit = 0;
    bool fCheckCredit = ComputeSidechainTxInputCredit(tx, input_credit);
    assert(fCheckCredit);
    bool fCheckResult = tx.IsCoinBase() || Consensus::CheckTxInputs(tx, dummy_state, consensus_params, mempoolDuplicate, spendheight, txfee, input_credit);
    assert(fCheckResult);
    UpdateCoins(tx, mempoolDuplicate, std::numeric_limits<int>::max());
}

void CTxMemPool::check(const CCoinsViewCache *pcoins, const Consensus::Params& consensus_params) const
{
    LOCK(cs);
    ...
    for (indexed_transaction_set::const_iterator it = mapTx.begin(); it != mapTx.end(); it++) {
        ...
        if (fDependsWait)
            ...
            CheckInputsAndUpdateCoins(tx, consensus_params, mempoolDuplicate, spendheight);
        }
    }
    ...
    while (!waitingOnDependants.empty()) {
        ...
        if (!mempoolDuplicate.HaveInputs(entry->GetTx())) {
            ...
            CheckInputsAndUpdateCoins(entry->GetTx(), consensus_params, mempoolDuplicate, spendheight);
...
{
    AssertLockHeld(cs);
    return mapValiditySidechainDepositById.count(key) != 0;
}

bool CTxMemPool::HasValiditySidechainReclaim(const std::pair<uint8_t, uint256>& key) const
{
    AssertLockHeld(cs);
...
bool CTxMemPool::HasValiditySidechainEscapeExit(const std::pair<uint8_t, uint256>& key) const
{
    AssertLockHeld(cs);
    return mapValiditySidechainEscapeExitById.count(key) != 0;
}
