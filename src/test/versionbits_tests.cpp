...
static void check_computeblockversion_bip8(const Consensus::Params& params, Consensus::DeploymentPos dep)
{
    ...
    bool lockin_on_timeout = params.vDeployments[dep].lockin_on_timeout;
    ...
    {
        // In the first chain, we don't set the bit, and we hit timeout.
        ...
        if (lockin_on_timeout) {
            // LOT=true: timeout transitions to LOCKED_IN for one full period, then ACTIVE.
            ...
            // LOT=false: timeout transitions directly to FAILED at the boundary.
            lastBlock = chain.Mine(nTimeoutHeight, nTime, VERSIONBITS_LAST_OLD_BLOCK_VERSION).Tip();
            BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, params) & bit_mask, 0);
            BOOST_CHECK_EQUAL(VersionBitsState(lastBlock, params, dep, versionbitscache), ThresholdState::FAILED);

            for (uint32_t i = 1; i < nMinerConfirmationWindow; i++) {
                lastBlock = chain.Mine(nTimeoutHeight + i, nTime, VERSIONBITS_TOP_BITS | bit_mask).Tip();
                BOOST_CHECK_EQUAL(ComputeBlockVersion(lastBlock, params) & bit_mask, 0);
                BOOST_CHECK_EQUAL(VersionBitsState(lastBlock, params, dep, versionbitscache), ThresholdState::FAILED);
            }
        }
