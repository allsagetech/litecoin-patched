...
{
    // Reuse the Litecoin testnet genesis until this fork defines and solves a
...
public:
    explicit CSigNetParams(const CChainParams::Options& options) {
        ...
        // Keep signet cheap to mine so local and CI smoke tests can produce
        // blocks quickly without depending on long scrypt brute-force runs.
    ...
    for (const std::string& strDeployment : args.GetArgs("-vbparams")) {
        ...
        for (int j=0; j < (int)Consensus::MAX_VERSION_BITS_DEPLOYMENTS; ++j) {
            if (vDeploymentParams[0] == VersionBitsDeploymentInfo[j].name) {
                ...
                if (!has_timeout_height) {
                    nTimeoutHeight = consensus.vDeployments[j].nTimeoutHeight;
                }

                // Height-based activation (BIP8) MUST align to period boundaries.
                if (nStartTime == 0 && nTimeout == 0) {
                    if (consensus.nMinerConfirmationWindow <= 0) {
                        throw std::runtime_error("Invalid confirmation window for BIP8 deployment");
                    }
                    if (nStartHeight < 0 || nTimeoutHeight <= nStartHeight) {
                        throw std::runtime_error(strprintf(
                            "Invalid BIP8 heights for %s: start_height=%lld timeout_height=%lld",
                            vDeploymentParams[0],
                            (long long)nStartHeight,
                            (long long)nTimeoutHeight));
                    }
                    if ((nStartHeight % consensus.nMinerConfirmationWindow) != 0 ||
                        (nTimeoutHeight % consensus.nMinerConfirmationWindow) != 0) {
                        throw std::runtime_error(strprintf(
                            "BIP8 heights for %s must align to %u-block boundaries: start_height=%lld timeout_height=%lld",
                            vDeploymentParams[0],
                ...
                found = true;
                LogPrintf(
                    "Setting version bits activation parameters for %s to start=%lld, timeout=%lld, start_height=%lld, timeout_height=%lld\n",
                    vDeploymentParams[0],
...
{
    const CChainParams::Options options = BuildChainOptions(args, chain);
