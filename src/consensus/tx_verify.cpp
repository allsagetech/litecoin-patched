    ...
    CAmount nInputValueCredit)
{
    ...
    if (!MoneyRange(nInputValueCredit)) {
        return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-txns-credit-outofrange");
    }

    const CAmount value_out = tx.GetValueOut();
    if (nValueIn + nInputValueCredit < value_out) {
        return state.Invalid(TxValidationResult::TX_CONSENSUS, "bad-txns-in-belowout",
            strprintf("value in (%s) + credit (%s) < value out (%s)",
                FormatMoney(nValueIn), FormatMoney(nInputValueCredit), FormatMoney(value_out)));
    }

    const CAmount value_deficit = std::max<CAmount>(0, value_out - nValueIn);
    const CAmount credit_used = std::min(nInputValueCredit, value_deficit);

    // Tally transaction fees. Credit is only used to cover deficits.
    CAmount txfee_aux = nValueIn - value_out + credit_used;
